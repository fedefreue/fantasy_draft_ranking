...
def calculatePoints(rawStats: pd.DataFrame(), points: dict):
    rawStats["waffleHousePoints"] = (
        points["Pass Yds"] * rawStats["PassingYds"]
        + points["Pass TD"] * rawStats["PassingTD"]
        + points["Int"] * rawStats["Int"]
        + points["Rush Att"] * rawStats["RushingAtt"]
        + points["Rush TD"] * rawStats["RushingTD"]
        + points["Rec"] * rawStats["Rec"]
        + points["Rec Yds"] * rawStats["ReceivingYds"]
        + points["Rec TD"] * rawStats["ReceivingTD"]
        + points["Fum Lost"] * rawStats["FumblesLost"]
    )
    ...
    rawStats["waffleHousePointsBefore"] = rawStats.groupby("Player")[
        "waffleHousePoints"
    ].shift(1)
...
def createRanks(
    ...
    tableForRank_withPlayers = pd.DataFrame(
        tableForRank,
        columns=[
            "Player",
            "Pos",
    ...
    for i in range(1, layer1 + 1):
        for j in range(1, min(i, layer2 + 1)):
            ...
            evalTable.iloc[-1, 2], currentModel = trainModel(
                i, j, modelFeatures, modelY
            )
            ...
            if evalTable.iloc[-1, 2] > best_r2:
                ...
                tableForRank["predictedPoints"] = bestModel.predict(
                    tableForRank, verbose=0
                )

                # Join tableForRank with tableForRank_withPlayers on index and add Player and Pos columns to tableForRank
                tableForRank = tableForRank.join(
                    tableForRank_withPlayers[["Player", "Pos"]], how="left"
                )
...
finalTable = pd.concat(
    [pd.read_csv(f) for f in glob.glob("finalRank_*.csv")], ignore_index=True
)
