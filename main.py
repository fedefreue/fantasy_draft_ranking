...
class GUI(object):
    ...
    def connectElementsFrame(self, frame: ttk.Frame):
        ...
        self.connectButton = ttk.Button(
            frame, text="Connect", command=self._connect, width=8
        )
        self.connectFileEntry = ttk.Entry(
            frame, width=20, textvariable=self.connectFile
        )
    ...
    def applyElementsFrame(self, frame: ttk.Frame):
        ...
        self.applyButton = ttk.Button(
            frame, text="Rank Players", command=self._rankPlayers, width=8
        )
        self.applyPositionEntry = ttk.Entry(
            frame, width=20, textvariable=self.applyPositionList
        )
        self.applyLabel = ttk.Label(frame, text="Positions:", anchor="e", width=12)

        self.loadButton = ttk.Button(
            frame, text="Load Model", command=self._loadModel, width=8
        )
        self.saveButton = ttk.Button(
            frame, text="Save Model", command=self._saveModel, width=8
        )
    ...
    def trainElementsFrame(self, frame: ttk.Frame):
        ...
        self.trainButton = ttk.Button(
            frame, text="Train Model", command=self._trainModel, width=8
        )
        self.trainInputLabel = ttk.Label(frame, text="Training Inputs", borderwidth=5)
        self.trainEntrySeasonList = ttk.Entry(
            frame, width=20, textvariable=self.trainSeasonList
        )
        self.trainPositionsList = ttk.Entry(
            frame, width=20, textvariable=self.trainPositionList
        )
    ...
    def _trainModel(self):
        ...
        masterTable = playerData.generateTrainingTable(
            seasonList, self.league, points, positionList
        )
